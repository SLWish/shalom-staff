...
export async function handler(event) {
  ...
  if (!isValidAccessSignature(subscriberId, expiresAt, signature)) {
    return redirect(`${siteUrl}/defeat-alert/?access=invalid`)
  }

  try {
    const [subscriber] = await selectRows(`defeat_subscribers?select=id&id=eq.${encodeURIComponent(subscriberId)}&limit=1`)
    if (!subscriber) return redirect(`${siteUrl}/defeat-alert/?access=invalid`)
    ...
    return redirect(`${siteUrl}/defeat-alert/manage?token=${encodeURIComponent(manageToken)}`)
  } catch (error) {
    console.error('[defeat-access]', error)
    return redirect(`${siteUrl}/defeat-alert/?access=error`)
  }
}
